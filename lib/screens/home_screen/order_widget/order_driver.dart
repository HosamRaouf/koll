...
class _OrderDriverState extends State<OrderDriver> {
  ...
  Widget build(BuildContext context) {
    ...
    driverId != "" ? {
      driverIndex = restaurantData.drivers.indexWhere((element) => element.id == driverId),
      driver = restaurantData.drivers[driverIndex],
      print(driver.toJson())
    } : {
      driver = DriverModel(id: "", firestoreId: "",  name: widget.order.driverName, phoneNumber: widget.order.driverPhoneNumber, image: widget.order.driverImage, orders: [], rates: [])
    };


    return Container(
      decoration: cardDecoration.copyWith(borderRadius: BorderRadius.circular(widget.radius)),
        ...
        onTap: () {
          driver.id != ""
              ...
              builder: (context) {
                ...
                return AddDriverDialog(phoneNumberController: phoneNumberController, nameController: nameController, imagePath: imagePath, imageWidget: imageWidget, selectImage: (url) {
                  setState(() {
                    imagePath = url;
